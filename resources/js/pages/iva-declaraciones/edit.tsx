...
import { type BreadcrumbItem, type Cliente, type IvaDeclaracion } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Editar',
        href: '#', // This will be dynamic based on the declaration
    },
];

interface Props {
    declaracion: IvaDeclaracion;
    clientes: Cliente[];
}

export default function IvaDeclaracionEdit({ declaracion, clientes }: Props) {
    const { data, setData, submit, errors, processing } = IvaDeclaracionController().update(declaracion.id);

    useEffect(() => {
        setData({
            cliente_id: declaracion.cliente_id,
            periodo_inicio: declaracion.periodo_inicio,
            periodo_fin: declaracion.periodo_fin,
            fecha_presentacion: declaracion.fecha_presentacion || '',
            monto_a_pagar: declaracion.monto_a_pagar || '',
            plazo: declaracion.plazo,
            cantidad_cuotas: declaracion.cantidad_cuotas || 1,
            dia_pago: declaracion.dia_pago || '',
        });
    }, [declaracion]);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Editar Declaración de IVA" />
            <div className="max-w-2xl mx-auto">
                <div className="rounded-md border p-6">
                    <h2 className="text-2xl font-semibold mb-4">Editar Declaración de IVA</h2>
